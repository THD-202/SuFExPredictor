...
def load_data(data_name, fp_name):
	...
	for train_index,val_index in kf.split(X_train, Y_train):
		...
		params = {'nthread': 8,  # 进程数
              'max_depth': 11,  # 最大深度
              'min_child_samples': 28,  # 树的数量
              'learning_rate': 0.25823859310798225,  # 学习率
              'num_leaves': 2048,  # 终点节点最小样本占比的和
              'subsample_for_bin': 1000,  # 样本列采样
              'reg_alpha': 0.8300219846523877,  # L1 正则化
              'reg_lambda': 0.3522383690153793,  # L2 正则化
	...
	pred1 = pd.DataFrame(model.predict(X_test))
	pred1.to_csv('%s.csv'%'lgbtest', header=None, index=False)
